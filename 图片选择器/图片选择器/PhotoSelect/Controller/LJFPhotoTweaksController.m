...
@interface LJFPhotoTweaksController ()

@property (strong, nonatomic) LJFPhotoTweakView *photoView;

@end

@implementation LJFPhotoTweaksController
...
- (void)setupSubviews
{
    self.photoView = [[LJFPhotoTweakView alloc] initWithFrame:self.view.bounds image:self.image maxRotationAngle:self.maxRotationAngle];
...
- (void)cancelBtnTapped
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(photoTweaksControllerDidCancel:)]) {
...
//    [self.delegate photoTweaksController:self didFinishWithCroppedImage:image];
    ...
    if (picker.singleFinishBlock) {
        picker.singleFinishBlock(image);
